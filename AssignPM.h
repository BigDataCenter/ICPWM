...
{//assign BW to each edge
    ...
    for(i = 0; i < MODLINK; i++)
    {
        ...
        else if (MPC[E[i].from].cloud != MPC[E[i].to].cloud)
        {
            ...
            E[i].BW        = BW[BWTypeBetweenClouds[MPC[E[i].from].cloud][MPC[E[i].to].cloud]].value/outdegree;//assign BW inter-cloud
            E[i].price    = BW[BWTypeBetweenClouds[MPC[E[i].from].cloud][MPC[E[i].to].cloud]].price;            //assign price of BW inter-cloud
            UpdateEdgeChanges(i);
        }
    }
}
